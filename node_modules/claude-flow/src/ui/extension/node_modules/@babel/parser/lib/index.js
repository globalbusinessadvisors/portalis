...
function toParseErrorConstructor({
  ...
  return function constructor(loc, details) {
    ...
    defineHidden(_error, "clone", function clone(overrides = {}) {
      var _overrides$loc;
      const { // TODO: Remove if unused
        line,
        column,
...
var estree = superClass => class ESTreeParserMixin extends superClass {
  ...
  parseBigIntLiteral(value) {
    let bigInt; // TODO: Remove if unused
    try {
      bigInt = BigInt(value);
    } catch (_unused) {
      bigInt = null;
...
function isIteratorStart(current, _next, next2) {
...
var flow = superClass => class FlowParserMixin extends superClass {
  ...
  flowParseObjectType({
    ...
    let endDelim; // TODO: Remove if unused
    let exact; // TODO: Remove if unused
    let inexact = false;
  ...
  flowParsePrimaryType() {
    ...
    const node = this.startNode();
    let tmp; // TODO: Remove if unused
    let type; // TODO: Remove if unused
  ...
  parseObjPropValue(prop, startLoc, isGenerator, isAsync, isPattern, isAccessor, refExpressionErrors) {
    ...
    let typeParameters; // TODO: Remove if unused
  ...
  flowEnumCheckExplicitTypeMismatch(loc, _context, expectedType) {
    const { // TODO: Remove if unused
      explicitType
    } = context;
...
var jsx = superClass => class JSXParserMixin extends superClass {
  ...
  jsxParseAttributeValue() {
    let node; // TODO: Remove if unused
    switch (this.state.type) {
      case 5:
        node = this.startNode();
  ...
  getTokenFromCode(code) {
    ...
    if (_context === types.j_expr) {
      this.jsxReadToken();
      return;
    }
    if (_context === types.j_oTag || context === types.j_cTag) {
...
function readEscapedChar(input, pos, lineStart, curLine, inTemplate, errors) {
  ...
  switch (ch) {
    ...
    case 120:
      {
        let code; // TODO: Remove if unused
        ({
          code,
          pos
...
class Tokenizer extends CommentsParser {
  ...
  skipBlockComment(commentEnd) {
    let startLoc; // TODO: Remove if unused
  ...
  getTokenFromCode(code) {
    switch (code) {
      ...
      case 48:
        {
          const next = this.input.charCodeAt(this.state.pos + 1);
          if (_next === 120 || next === 88) {
            this.readRadixNumber(16);
            return;
          }
          if (_next === 111 || next === 79) {
  ...
  readRegexp() {
    ...
    let escaped, inClass; // TODO: Remove if unused
    let { // TODO: Remove if unused
      pos
    } = this.state;
    for (;; ++pos) {
...
class ClassScopeHandler {
  ...
  usePrivateName(name, loc) {
    let classScope; // TODO: Remove if unused
...
class ExpressionScopeHandler {
  ...
  recordArrowParameterBindingError(_error, node) {
    const { // TODO: Remove if unused
      stack
    } = this;
...
class LValParser extends NodeUtils {
  ...
  checkLVal(expression, ancestor, binding = 64, checkClashes = false, strictModeChanged = false, hasParenthesizedAncestor = false) {
    ...
    let key, isParenthesizedExpression; // TODO: Remove if unused
...
var typescript = superClass => class TypeScriptParserMixin extends superClass {
  ...
  tsParseEntityName(flags) {
    let entity; // TODO: Remove if unused
  ...
  tsParseTupleElementType() {
    ...
    let labeled; // TODO: Remove if unused
    let label; // TODO: Remove if unused
    let optional; // TODO: Remove if unused
    let type; // TODO: Remove if unused
    ...
    if (labeled) {
      let labeledNode; // TODO: Remove if unused
      if (label) {
  ...
  tsParseDeclaration(node, value, _next, decorators) {
    switch (value) {
      case "abstract":
  ...
  parseSubscript(base, startLoc, noCalls, state) {
    ...
    if (this.match(47) || this.match(51)) {
      let missingParenErrorLoc; // TODO: Remove if unused
  ...
  parseExprOp(left, leftStartLoc, minPrec) {
    let isSatisfies; // TODO: Remove if unused
  ...
  parseImport(node) {
    ...
    let importNode; // TODO: Remove if unused
  ...
  parseMaybeAssign(refExpressionErrors, afterLeftParse) {
    ...
    let state; // TODO: Remove if unused
    let jsx; // TODO: Remove if unused
    let typeCast; // TODO: Remove if unused
    if (this.hasPlugin("jsx") && (this.match(143) || this.match(47))) {
      state = this.state.clone();
      jsx = this.tryParse(() => super.parseMaybeAssign(refExpressionErrors, afterLeftParse), state);
      if (!jsx.error) return jsx.node;
  ...
  parseTypeOnlyImportExportSpecifier(node, isImport, isInTypeOnlyImportExport) {
    ...
    let rightOfAs; // TODO: Remove if unused
...
class ExpressionParser extends LValParser {
  ...
  parseMaybeAssign(refExpressionErrors, afterLeftParse) {
    ...
    let ownExpressionErrors; // TODO: Remove if unused
    if (refExpressionErrors) {
  ...
  parseParenAndDistinguishExpression(canBeArrow) {
    ...
    let first = true;
    let spreadStartLoc; // TODO: Remove if unused
    let optionalCommaStartLoc; // TODO: Remove if unused
    while (!this.match(11)) {
      if (first) {
        first = false;
      } else {
  ...
  parseObjectLike(close, isPattern, isRecord, refExpressionErrors) {
    ...
    while (!this.match(close)) {
      ...
      let prop; // TODO: Remove if unused
      if (isPattern) {
  ...
  parseIdentifierName(liberal) {
    let name; // TODO: Remove if unused
    const { // TODO: Remove if unused
      startLoc,
      type
    } = this.state;
  ...
  withTopicBindingContext(_callback) {
  ...
  withSmartMixTopicForbiddingContext(_callback) {
    if (this.hasPlugin(["pipelineOperator", {
      proposal: "smart"
    }])) {
      const outerContextTopicState = this.state.topicContext;
  ...
  withSoloAwaitPermittingContext(_callback) {
    ...
    if (typeof type === "number") {
      {
        ...
        if (tokenIsTemplate(type)) {
          ...
          let startToken; // TODO: Remove if unused
          ...
          let templateValue, templateElementEnd, templateElementEndLoc, endToken; // TODO: Remove if unused
          if (type === 24) {
...
class StatementParser extends ExpressionParser {
  ...
  parseProgram(program, end = 140, sourceType = this.options.sourceType) {
    ...
    let finishedProgram; // TODO: Remove if unused
    if (end === 140) {
  ...
  parseDecorator() {
    ...
    if (this.hasPlugin("decorators")) {
      const startLoc = this.state.startLoc;
      let expr; // TODO: Remove if unused
      if (this.match(10)) {
        const startLoc = this.state.startLoc;
        this.next();
  ...
  parseForStatement(node) {
    ...
    const startsWithLet = this.isContextual(100);
    {
      ...
      if (this.match(74) || this.match(75) || isLetOrUsing) {
        ...
        let kind; // TODO: Remove if unused
  ...
  parseExport(node, decorators) {
    ...
    let hasDeclaration; // TODO: Remove if unused
    ...
    if (this.isJSONModuleImport(node) && node.type !== "ExportAllDeclaration") {
      ...
      if (specifiers != null) {
        const nonDefaultNamedSpecifier = specifiers.find(specifier => {
          let imported; // TODO: Remove if unused
  ...
  maybeParseImportAttributes(node) {
    let attributes; // TODO: Remove if unused
    {
      var useWith = false;
    }
    if (this.match(76)) {
...
function getParser(options, input) {
  ...
  if (options != null && options.plugins) {
    for (const plugin of options.plugins) {
      let name, opts; // TODO: Remove if unused
