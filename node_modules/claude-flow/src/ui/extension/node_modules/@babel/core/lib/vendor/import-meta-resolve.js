...
let userStackTraceLimit; // TODO: Remove if unused
...
const captureLargerStackTrace = hideStackFrames(function (_error) {
...
function read(jsonPath, {
  ...
  let string; // TODO: Remove if unused
  try {
...
function defaultGetFormatWithoutErrors(url, _context) {
...
function legacyMainResolve(packageJsonUrl, packageConfig, base) {
  let guess; // TODO: Remove if unused
...
function finalizeResolution(resolved, base, preserveSymlinks) {
  ...
  let filePath; // TODO: Remove if unused
  try {
...
function resolvePackageTarget(packageJsonUrl, target, subpath, packageSubpath, base, pattern, internal, isPathMap, conditions) {
  ...
  if (Array.isArray(target)) {
    ...
    let lastException; // TODO: Remove if unused
    let i = -1;
    while (++i < targetList.length) {
      const targetItem = targetList[i];
      let resolveResult; // TODO: Remove if unused
      try {
...
function packageImportsResolve(name, base, conditions) {
  ...
  let packageJsonUrl; // TODO: Remove if unused
...
function packageResolve(specifier, base, conditions) {
  ...
  let lastPath; // TODO: Remove if unused
  do {
...
function moduleResolve(specifier, base, conditions, preserveSymlinks) {
  ...
  let resolved; // TODO: Remove if unused
...
function defaultResolve(specifier, context = {}) {
  ...
  let parsedParentURL; // TODO: Remove if unused
  if (parentURL) {
    try {
  ...
  let parsed; // TODO: Remove if unused
  let protocol; // TODO: Remove if unused
  try {
