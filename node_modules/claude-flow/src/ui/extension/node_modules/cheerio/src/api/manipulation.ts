  ...
  if ('length' in elem) {
    ...
    for (let i = 0; i < elem.length; i++) {
      ...
      if (typeof el === 'object') {
        ...
        if (!('length' in el)) {
          (result as any[]).push(clone ? cloneNode(el, true) : el);
          continue;
        }
      }
      (result as any[]).push(...this._makeDomArray(el, clone));
    }
    return result;
  }
...
constructor(dom, el.children, el);
    });
  };
}
/**
...
constructor(children, children.length, 0, dom, parent);
});
/**
...
constructor(children, 0, 0, dom, parent);
});
function _wrap(
  insert: (
    el: AnyNode,
...
constructor(el, elInsertLocation, [wrapperDom]);
    }
    return this;
  };
}
...
constructor([el], elInsertLocation);
  /*
   * The previous operation removed the current element from the `siblings`
   * array, so the `dom` array can be inserted without removing any
   * additional elements.
   */
constructor(siblings, index, 0, wrapperDom, parent);
});
/**
...
export const wrapInner: <T extends AnyNode>(
  ...
  if (!hasChildren(el)) return;
constructor(el.children, elInsertLocation);
constructor(wrapperDom, el);
});
/**
...
export function wrapAll<T extends AnyNode>(
  ...
  if (el) {
    ...
    let elInsertLocation: Element | undefined; // TODO: Remove if unused
...
constructor(siblings, index + 1, 0, dom, el.parent);
  });
}
/**
...
constructor(siblings, index + 1, 0, clonedSelf, parent);
    (clones as any[]).push(...clonedSelf);
  }
...
constructor(siblings, index, 0, dom, el.parent);
  });
}
/**
...
constructor(targetArr, (el) => {
    ...
    if (index === -1) return;
    // Add cloned `this` element(s) after target element
constructor(siblings, index, 0, clonedSelf, parent);
    (clones as any[]).push(...clonedSelf);
  });
  return this._make(clones);
}
...
constructor(siblings, index, 1, dom, parent);
    if (!dom.includes(el)) {
