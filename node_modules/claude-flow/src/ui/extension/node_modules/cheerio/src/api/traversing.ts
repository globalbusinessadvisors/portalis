...
export const parents: <T extends AnyNode>(
  ...
  (elem) => {
    ...
    while (elem.parent && !isDocument(elem.parent)) {
      (matched as any[]).push(elem.parent as Element);
      elem = elem.parent;
    }
    return matched;
  },
...
export function closest<T extends AnyNode>(
  ...
  const selectFn =
    typeof selector === 'string'
      ...
      : getFilterFn(selector);
constructor(this, (elem: AnyNode | null) => {
...
export const nextAll: <T extends AnyNode>(
  ...
  while (elem.next) {
    elem = elem.next;
    if (isTag(elem)) (matched as any[]).push(elem);
  }
  return matched;
}, _removeDuplicates);
/**
...
export const siblings: <T extends AnyNode>(
  ...
  (elem) =>
constructor(elem).filter((el): el is Element => isTag(el) && el !== elem),
  uniqueSort,
);
/**
...
export function contents<T extends AnyNode>(
  ...
  const elems = this.toArray().reduce<AnyNode[]>(
    (newElems, elem) =>
constructor(elem) ? newElems.concat(elem.children) : newElems,
    [],
  );
...
constructor(this.toArray(), match, this.options.xmlMode, this._root?.[0]),
  );
}
...
export function index<T extends AnyNode>(
  ...
  let $haystack: Cheerio<AnyNode>; // TODO: Remove if unused
  let needle: AnyNode; // TODO: Remove if unused
