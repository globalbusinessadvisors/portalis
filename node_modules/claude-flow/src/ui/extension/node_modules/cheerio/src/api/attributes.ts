...
constructor(el, objName, objValue);
        }
      } else {
    ...
    return domEach(this, (el) => {
      ...
      if (typeof name === 'object') {
        for (const key of Object.keys(name)) {
          const val = name[key];
constructor(el, key, val, this.options.xmlMode);
        }
      } else {
constructor(el, name, value, this.options.xmlMode);
      }
    });
  }
...
constructor(el, 'class', setClass.trim());
    } else {
constructor(el, 'class', classNames.join(' ').trim());
    }
  }
...
export function toggleClass<T extends AnyNode, R extends ArrayLike<T>>(
  ...
  for (let i = 0; i < numElements; i++) {
    ...
    for (let j = 0; j < numClasses; j++) {
      ...
      if (state >= 0 && index === -1) {
        (elementClasses as any[]).push(classNames[j]);
