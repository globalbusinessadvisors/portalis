...
(function (global, factory) {
    ...
    const invoke = (_callback, eventName, logger, telemetryClient, correlationId) => {
        return (...args) => {
     ...
     */
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const invokeAsync = (_callback, eventName, logger, telemetryClient, correlationId) => {
        return (...args) => {
            logger.trace(`Executing function ${eventName}`);
            const inProgressEvent = telemetryClient?.startMeasurement(eventName, correlationId);
            if (correlationId) {
    ...
    class NetworkError extends AuthError {
        constructor(_error, httpStatus, responseHeaders) {
     ...
     */
    function createNetworkError(_error, httpStatus, responseHeaders, additionalError) {
     ...
     */
    function startContext(_event, abbreviations, stack) {
        if (!stack) {
            return;
        }
        stack.push({
     ...
     */
    function endContext(_event, abbreviations, stack, _error) {
        if (!stack?.length) {
            return;
        }
        const peek = (stack) => {
     ...
     */
    function addError(_error, logger, _event, stackMaxSize = 5) {
    ...
    class PerformanceClient {
         ...
         * @param {PerformanceEvent} event
         */
        cacheEventByCorrelationId(_event) {
            const rootEvent = this.eventsByCorrelationId.get(event.correlationId);
            if (rootEvent) {
     ...
     * @returns
     */
    async function deriveKey(baseKey, nonce, _context) {
     ...
     * @param rawData
     */
    async function encrypt(baseKey, rawData, _context) {
     ...
     * @returns
     */
    async function decrypt(baseKey, nonce, _context, encryptedData) {
    ...
    class AsyncMemoryStorage {
        ...
        handleDatabaseAccessError(_error) {
    ...
    class EventHandler {
         ...
         */
        addEventCallback(_callback, eventTypes, callbackId) {
            if (typeof window !== "undefined") {
         ...
         * @param error
         */
        emitEvent(eventType, interactionType, payload, _error) {
            const message = {
                eventType: eventType,
         ...
         * @param event
         */
        invokeCrossTabCallbacks(_event) {
     ...
     */
    function isFatalNativeAuthError(_error) {
        if (error.ext &&
            error.ext.status &&
    ...
    class PlatformAuthExtensionHandler {
        ...
        async sendHandshakeRequest() {
            ...
            this.messageChannel.port1.onmessage = (_event) => {
    ...
    class StandardController {
         ...
         * @param callback
         */
        addEventCallback(_callback, eventTypes) {
