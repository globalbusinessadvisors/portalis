...
const networkRequestViaProxy = <T>(
    ...
    return new Promise<NetworkResponse<T>>((resolve, reject) => {
        ...
        if (timeout) {
            request.on("timeout", () => {
                request.destroy();
constructor(new Error("Request time out"));
            });
        }
        request.end();
...
constructor(networkResponse);
            ...
            socket.on("error", (chunk) => {
                request.destroy();
                socket.destroy();
constructor(new Error(chunk.toString()));
            });
        });
...
const networkRequestViaHttps = <T>(
    ...
    return new Promise<NetworkResponse<T>>((resolve, reject) => {
        let request: http.ClientRequest; // TODO: Remove if unused
...
const parseBody = (
     ...
     */
    let parsedBody; // TODO: Remove if unused
    try {
        parsedBody = JSON.parse(body);
    } catch (_error) {
        let errorType; // TODO: Remove if unused
        let errorDescriptionHelper; // TODO: Remove if unused
        if (
