...
function string_lstrip(string, prefix_chars) {
    let idx = 0
    while (idx < string.length && prefix_chars.includes(string[idx])) idx++
    return idx ? string.slice(idx) : string
}
function string_split(string, sep, maxsplit) {
    ...
    return class _AttributeHolder extends cls {
        [util.inspect.custom]() {
            ...
            for (let arg of this._get_args()) {
                argstrings.push(repr(arg))
            }
            for (let [ name, value ] of this._get_kwargs()) {
                if (/^[a-z_][a-z0-9_$]*$/i.test(name)) {
                    argstrings.push(sub('%s=%r', name, value))
                } else {
                    star_args[name] = value
                }
            }
            if (Object.keys(star_args).length) {
                argstrings.push(sub('**%s', repr(star_args)))
            }
            return sub('%s(%s)', type_name, argstrings.join(', '))
        }
        toString() {
...
const HelpFormatter = _camelcase_alias(_callable(class HelpFormatter {
        ...
        // if optionals and positionals are available, calculate usage
            ...
            if (prefix.length + usage.length > text_width) {
                ...
                let get_lines = (parts, indent, prefix = undefined) => {
                    let lines = []
                    let line = []
                    let line_len // TODO: Remove if unused
                    if (prefix !== undefined) {
                ...
                let lines // TODO: Remove if unused
    ...
    _format_actions_usage(actions, groups) {
        ...
        for (let [ i, action ] of Object.entries(actions)) {
            ...
            } else if (!action.optionstrings.length) {
            ...
            // produce the first way to invoke the option in brackets
                ...
                let optionstring = action.optionstrings[0]
                let part // TODO: Remove if unused
                ...
                //    -s ARGS or --long ARGS
                    ...
                    let argsstring = this._format_args(action, default_value)
                    part = sub('%s %s', optionstring, argsstring)
                }
    ...
    _format_action(action) {
        ...
        let indent_first // TODO: Remove if unused
    ...
    _format_action_invocation(action) {
        if (!action.optionstrings.length) {
            ...
            //    -s ARGS, --long ARGS
                ...
                let default_value = this._get_default_metavar_for_optional(action)
                let argsstring = this._format_args(action, default_value)
                for (let optionstring of action.optionstrings) {
                    parts.push(sub('%s %s', optionstring, argsstring))
                }
            }
            return parts.join(', ')
        }
    ...
    _format_args(action, default_metavar) {
        ...
        if (action.nargs === undefined) {
            ...
            let formats // TODO: Remove if unused
            try {
    ...
    _expand_help(action) {
        ...
        // end
        return sub(this._get_helpstring(action), params)
    }
...
HelpFormatter.prototype._Section = _callable(class _Section {
    ...
    format_help() {
        ...
        let heading // TODO: Remove if unused
    ...
    _get_helpstring(action) {
        ...
        if (!action.help.includes('%(default)') && !action.help.includes('%(defaultValue)')) {
            if (action.default !== SUPPRESS) {
                ...
                if (action.optionstrings.length || defaulting_nargs.includes(action.nargs)) {
...
function _get_action_name(argument) {
    if (argument === undefined) {
        return undefined
    } else if (argument.optionstrings.length) {
...
const ArgumentError = _callable(class ArgumentError extends Error {
    ...
    str() {
        let format // TODO: Remove if unused
...
const BooleanOptionalAction = _camelcase_alias(_callable(class BooleanOptionalAction extends Action {
    constructor() {
        ...
        for (let optionstring of optionstrings) {
            _optionstrings.push(optionstring)
    ...
    call(parser, namespace, values, optionstring = undefined) {
        if (this.optionstrings.includes(optionstring)) {
            setattr(namespace, this.dest, !optionstring.startsWith('--no-'))
        }
    }
    format_usage() {
...
const _SubParsersAction = _camelcase_alias(_callable(class _SubParsersAction extends Action {
    ...
    call(parser, namespace, values/*, optionstring = undefined*/) {
        ...
        let subnamespace // TODO: Remove if unused
        [ subnamespace, argstrings ] = parser.parse_known_args(argstrings, undefined)
        for (let [ key, value ] of Object.entries(subnamespace)) {
            setattr(namespace, key, value)
        }
        if (argstrings.length) {
            setdefault(namespace, _UNRECOGNIZED_ARGS_ATTR, [])
            getattr(namespace, _UNRECOGNIZED_ARGS_ATTR).push(...argstrings)
        }
    }
}))
_SubParsersAction.prototype._ChoicesPseudoAction = _callable(class _ChoicesPseudoAction extends Action {
    constructor(name, aliases, help) {
        ...
        super({ optionstrings: [], dest, help, metavar })
    }
})
...
const _ActionsContainer = _camelcase_alias(_callable(class _ActionsContainer {
    constructor() {
        ...
        this._negativenumber_matcher = /^-\d+$|^-\d*\.\d+$/
        // whether or not there are any optionals that look like negative
        // numbers -- uses a list so it can be shared and edited
        this._has_negativenumber_optionals = []
    }
    // ====================
    ...
    add_argument() {
         ...
         *  add_argument(optionstring, optionstring, ..., name=value, ...)
         */
        let [ // TODO: Remove if unused
            args,
            kwargs
    ...
    _add_action(action) {
        ...
        for (let optionstring of action.optionstrings) {
            this._optionstring_actions[optionstring] = action
        }
        // set the flag if any option strings look like negative numbers
        for (let optionstring of action.optionstrings) {
            if (this._negativenumber_matcher.test(optionstring)) {
                if (!this._has_negativenumber_optionals.length) {
                    this._has_negativenumber_optionals.push(true)
                }
            }
        }
    ...
    _get_positional_kwargs() {
        ...
        return Object.assign(kwargs, { dest, optionstrings: [] })
    }
    _get_optional_kwargs() {
        ...
        let optionstrings = []
        let long_optionstrings = []
        let optionstring // TODO: Remove if unused
        for (optionstring of args) {
            // error on strings that don't start with an appropriate prefix
            if (!this.prefix_chars.includes(optionstring[0])) {
                let args = {option: optionstring,
            ...
            if (optionstring.length > 1 && this.prefix_chars.includes(optionstring[1])) {
                long_optionstrings.push(optionstring)
            }
        }
        ...
        delete kwargs.dest
        if (dest === undefined) {
            let dest_optionstring // TODO: Remove if unused
            if (long_optionstrings.length) {
                dest_optionstring = long_optionstrings[0]
            } else {
                dest_optionstring = optionstrings[0]
            }
            dest = string_lstrip(dest_optionstring, this.prefix_chars)
            if (!dest) {
                let msg = 'dest= is required for options like %r'
                throw new TypeError(sub(msg, optionstring))
            }
    ...
    _check_conflict(action) {
        ...
        let confl_optionals = []
        for (let optionstring of action.optionstrings) {
            if (hasattr(this._optionstring_actions, optionstring)) {
                let confl_optional = this._optionstring_actions[optionstring]
                confl_optionals.push([ optionstring, confl_optional ])
            }
        }
        // resolve any conflicts
    ...
    _handle_conflict_error(action, conflicting_actions) {
        ...
        let conflictstring = conflicting_actions.map(([ optionstring/*, action*/ ]) => optionstring).join(', ')
        throw new ArgumentError(action, sub(message, conflictstring))
    }
    _handle_conflict_resolve(action, conflicting_actions) {
        // remove all conflicting options
        for (let [ optionstring, action ] of conflicting_actions) {
            // remove the conflicting option
            _array_remove(action.optionstrings, optionstring)
            delete this._optionstring_actions[optionstring]
...
const _ArgumentGroup = _callable(class _ArgumentGroup extends _ActionsContainer {
    constructor() {
        ...
        this._optionstring_actions = container._optionstring_actions
        this._defaults = container._defaults
        this._has_negativenumber_optionals =
            container._has_negativenumber_optionals
...
const ArgumentParser = _camelcase_alias(_callable(class ArgumentParser extends _AttributeHolder(_ActionsContainer) {
    ...
    add_subparsers() {
        ...
        // eslint-disable-next-line new-cap
        let action = new parsers_class(Object.assign({ optionstrings: [] }, kwargs))
    ...
    _get_optional_actions() {
        return this._actions.filter(action => action.optionstrings.length)
    }
    ...
    parse_args(args = undefined, namespace = undefined) {
        let argv // TODO: Remove if unused
    ...
    _parse_known_args(argstrings, namespace) {
        // replace arg strings that are file references
        if (this.fromfile_prefix_chars !== undefined) {
            argstrings = this._read_args_from_files(argstrings)
        }
        ...
        let optionstring_indices = {}
        let argstring_pattern_parts = []
        let argstrings_iter = Object.entries(argstrings)[Symbol.iterator]()
        for (let [ i, argstring ] of argstrings_iter) {
            // all args after -- are non-options
            if (argstring === '--') {
                argstring_pattern_parts.push('-')
                for ([ i, argstring ] of argstrings_iter) {
                    argstring_pattern_parts.push('A')
                }
            // otherwise, add the arg to the arg strings
            // and note the index if it was an option
            } else {
                let option_tuple = this._parse_optional(argstring)
                let pattern // TODO: Remove if unused
                if (option_tuple === undefined) {
                    pattern = 'A'
                } else {
                    optionstring_indices[i] = option_tuple
                    pattern = 'O'
                }
                argstring_pattern_parts.push(pattern)
            }
        }
        // join the pieces together to form the pattern
        let argstrings_pattern = argstring_pattern_parts.join('')
        // converts arg strings to the appropriate and then takes the action
        let seen_actions = new Set()
        let seen_non_default_actions = new Set()
        let extras // TODO: Remove if unused
        let take_action = (action, argumentstrings, optionstring = undefined) => {
            seen_actions.add(action)
            let argument_values = this._get_values(action, argumentstrings)
            ...
            if (argument_values !== SUPPRESS) {
                action(this, namespace, argument_values, optionstring)
            }
        }
        // function to convert argstrings into an optional action
        let consume_optional = start_index => {
            // get the optional identified at this index
            let option_tuple = optionstring_indices[start_index]
            let [ action, optionstring, explicit_arg ] = option_tuple
            // identify additional optionals in the same arg string
            // (e.g. -xyz is the same as -x -y -z if no args are required)
            let action_tuples = []
            let stop // TODO: Remove if unused
            for (;;) {
                // if we found no optional action, skip it
                if (action === undefined) {
                    extras.push(argstrings[start_index])
                ...
                if (explicit_arg !== undefined) {
                    ...
                    let chars = this.prefix_chars
                    if (arg_count === 0 && !chars.includes(optionstring[1])) {
                        action_tuples.push([ action, [], optionstring ])
                        let char = optionstring[0]
                        optionstring = char + explicit_arg[0]
                        let new_explicit_arg = explicit_arg.slice(1) || undefined
                        let optionals_map = this._optionstring_actions
                        if (hasattr(optionals_map, optionstring)) {
                            action = optionals_map[optionstring]
                    ...
                    // successfully matched the option; exit the loop
                        ...
                        action_tuples.push([ action, args, optionstring ])
                        break
                ...
                // if successful, exit the loop
                    ...
                    let selected_patterns = argstrings_pattern.slice(start)
                    let arg_count = this._match_argument(action, selected_patterns)
                    stop = start + arg_count
                    let args = argstrings.slice(start, stop)
                    action_tuples.push([ action, args, optionstring ])
                    break
                }
            }
            ...
            for (let [ action, args, optionstring ] of action_tuples) {
                take_action(action, args, optionstring)
            }
            return stop
        }
        ...
        let consume_positionals = start_index => {
            ...
            let selected_pattern = argstrings_pattern.slice(start_index)
            ...
            for (let i = 0; i < positionals.length && i < arg_counts.length; i++) {
                ...
                let args = argstrings.slice(start_index, start_index + arg_count)
        ...
        extras = []
        let start_index = 0
        let max_optionstring_index = Math.max(-1, ...Object.keys(optionstring_indices).map(Number))
        while (start_index <= max_optionstring_index) {
            // consume any Positionals preceding the next option
            let next_optionstring_index = Math.min(
                // eslint-disable-next-line no-loop-func
                ...Object.keys(optionstring_indices).map(Number).filter(index => index >= start_index)
            )
            if (start_index !== next_optionstring_index) {
            ...
            if (!(start_index in optionstring_indices)) {
                let strings = argstrings.slice(start_index, next_optionstring_index)
                extras = extras.concat(strings)
                start_index = next_optionstring_index
            }
        ...
        extras = extras.concat(argstrings.slice(stop_index))
    ...
    _read_args_from_files(argstrings) {
        ...
        for (let argstring of argstrings) {
            ...
            if (!argstring || !this.fromfile_prefix_chars.includes(argstring[0])) {
                new_argstrings.push(argstring)
            // replace arguments referencing files with the file content
            } else {
                try {
                    let args_file = fs.readFileSync(argstring.slice(1), 'utf8')
                    let argstrings = []
                    ...
                    argstrings = this._read_args_from_files(argstrings)
                    new_argstrings = new_argstrings.concat(argstrings)
                } catch (err) {
                    this.error(err.message)
                }
            }
        ...
        return new_argstrings
    }
    convert_arg_line_to_args(arg_line) {
        return [arg_line]
    }
    _match_argument(action, argstrings_pattern) {
        // match the pattern for this action to the arg strings
        let nargs_pattern = this._get_nargs_pattern(action)
        let match = argstrings_pattern.match(new RegExp('^' + nargs_pattern))
    ...
    _match_arguments_partial(actions, argstrings_pattern) {
        ...
        for (let i of range(actions.length, 0, -1)) {
            ...
            let match = argstrings_pattern.match(new RegExp('^' + pattern))
            if (match !== null) {
    ...
    _parse_optional(argstring) {
        ...
        if (!this.prefix_chars.includes(argstring[0])) {
            return undefined
        }
        // if the option string is present in the parser, return the action
        if (argstring in this._optionstring_actions) {
            let action = this._optionstring_actions[argstring]
        ...
        if (argstring.includes('=')) {
            let [ optionstring, explicit_arg ] = string_split(argstring, '=', 1)
            if (optionstring in this._optionstring_actions) {
                let action = this._optionstring_actions[optionstring]
                return [ action, optionstring, explicit_arg ]
            }
        }
        // search through all possible prefixes of the option string
        // and all actions in the parser for possible interpretations
        let option_tuples = this._get_option_tuples(argstring)
        // if multiple actions match, the option string was ambiguous
        if (option_tuples.length > 1) {
            let options = option_tuples.map(([ /*action*/, optionstring/*, explicit_arg*/ ]) => optionstring).join(', ')
            let args = {option: argstring, matches: options}
        ...
        if (this._negativenumber_matcher.test(argstring)) {
            if (!this._has_negativenumber_optionals.length) {
                return undefined
            }
        }
    ...
    _get_option_tuples(optionstring) {
        ...
        // split at the '='
        let chars = this.prefix_chars
        if (chars.includes(optionstring[0]) && chars.includes(optionstring[1])) {
            if (this.allow_abbrev) {
                let option_prefix, explicit_arg // TODO: Remove if unused
                if (optionstring.includes('=')) {
                    [ option_prefix, explicit_arg ] = string_split(optionstring, '=', 1)
                } else {
                    option_prefix = optionstring
                    explicit_arg = undefined
                }
                for (let optionstring of Object.keys(this._optionstring_actions)) {
                    if (optionstring.startsWith(option_prefix)) {
                        let action = this._optionstring_actions[optionstring]
                        let tup = [ action, optionstring, explicit_arg ]
                        result.push(tup)
                    }
                }
            }
        ...
        // separate
        } else if (chars.includes(optionstring[0]) && !chars.includes(optionstring[1])) {
            let option_prefix = optionstring
            let explicit_arg = undefined
            let short_option_prefix = optionstring.slice(0, 2)
            let short_explicit_arg = optionstring.slice(2)
            for (let optionstring of Object.keys(this._optionstring_actions)) {
                if (optionstring === short_option_prefix) {
                    let action = this._optionstring_actions[optionstring]
                    let tup = [ action, optionstring, short_explicit_arg ]
                    result.push(tup)
                } else if (optionstring.startsWith(option_prefix)) {
                    let action = this._optionstring_actions[optionstring]
                    let tup = [ action, optionstring, explicit_arg ]
                    result.push(tup)
                }
        ...
        // shouldn't ever get here
            ...
            this.error(sub('unexpected option string: %s', optionstring))
        }
    ...
    _get_nargs_pattern(action) {
        ...
        let nargs_pattern // TODO: Remove if unused
    ...
    parse_known_intermixed_args(args = undefined, namespace = undefined) {
        ...
        let save_usage // TODO: Remove if unused
        try {
            save_usage = this.usage
            let remaining_args // TODO: Remove if unused
            try {
    ...
    _get_values(action, argstrings) {
        ...
        } else if (!argstrings.length && action.nargs === ZERO_OR_MORE &&
              !action.optionstrings.length) {
            if (action.default !== undefined) {
                value = action.default
            } else {
                value = argstrings
        ...
        } else if (argstrings.length === 1 && [undefined, OPTIONAL].includes(action.nargs)) {
            let argstring = argstrings[0]
            value = this._get_value(action, argstring)
            this._check_value(action, value)
        ...
        try {
            try {
                ...
                if (err instanceof TypeError &&
                    ...
                    // eslint-disable-next-line new-cap
                    result = new type_func(argstring)
                } else {
                    throw err
                }
            }
            ...
            // TypeErrors or ValueErrors also indicate errors
                ...
                let name = getattr(action.type, 'name', repr(action.type))
                let args = {type: name, value: argstring}
