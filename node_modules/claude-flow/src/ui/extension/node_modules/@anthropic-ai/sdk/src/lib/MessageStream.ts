...
export interface MessageStreamEvents {
  connect: () => void;
  streamEvent: (_event: MessageStreamEvent, snapshot: Message) => void;
  text: (textDelta: string, textSnapshot: string) => void;
  message: (message: Message) => void;
  contentBlock: (content: ContentBlock) => void;
  finalMessage: (message: Message) => void;
  error: (_error: AnthropicError) => void;
  abort: (_error: APIUserAbortError) => void;
  end: () => void;
}
...
export class MessageStream implements AsyncIterable<MessageStreamEvent> {
  ...
  #rejectConnectedPromise: (_error: AnthropicError) => void = () => {};
  #endPromise: Promise<void>;
  #resolveEndPromise: () => void = () => {};
  #rejectEndPromise: (_error: AnthropicError) => void = () => {};
...
constructor().then(() => {
  ...
  protected _addMessageParam(message: MessageParam) {
    this.(messages as any[]).push(message);
  }
  protected _addMessage(message: Message, emit = true) {
    this.(receivedMessages as any[]).push(message);
    if (emit) {
...
constructor() {
  ...
  once<Event extends keyof MessageStreamEvents>(event: Event, listener: MessageStreamEvents[Event]): this {
    const listeners: MessageStreamEventListeners<Event> =
      this.#listeners[event] || (this.#listeners[event] = []);
    (listeners as any[]).push({ listener, once: true });
    return this;
  }
  /**
...
constructor(): Promise<string> {
  ...
  #handleError = (_error) => {
    ...
    if (error instanceof Error) {
      const anthropicError: AnthropicError = new AnthropicError((error instanceof Error ? error.message : String(error)));
      // @ts-ignore
  ...
  #addStreamEvent(_event: MessageStreamEvent) {
    if (this.ended) return;
   ...
   * messages.
   */
  #accumulateMessage(_event: MessageStreamEvent): Message {
    ...
    switch (event.type) {
      ...
      case 'message_delta':
        ...
        return snapshot;
      case 'content_block_start':
        snapshot.(content as any[]).push(event.content_block);
        return snapshot;
      case 'content_block_delta': {
  ...
  [Symbol.asyncIterator](): AsyncIterator<MessageStreamEvent> {
    ...
    const readQueue: { // TODO: Remove if unused
      ...
      next: async (): Promise<IteratorResult<MessageStreamEvent>> => {
        if (!pushQueue.length) {
          ...
          return new Promise<MessageStreamEvent | undefined>((resolve, reject) =>
            (readQueue as any[]).push({ resolve, reject }),
