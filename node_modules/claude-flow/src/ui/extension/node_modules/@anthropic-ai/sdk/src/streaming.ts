...
constructor(
   ...
   */
constructor(): [Stream<Item>, Stream<Item>] {
      ...
      return {
        next: () => {
          if (queue.length === 0) {
            ...
            (left as any[]).push(result);
            (right as any[]).push(result);
          }
          return queue.shift()!;
        },
      };
...
constructor(): ReadableStream {
    const self = this;
    let iter: AsyncIterator<Item>; // TODO: Remove if unused
...
async function* iterSSEChunks(iterator: AsyncIterableIterator<Bytes>): AsyncGenerator<Uint8Array> {
  ...
  for await (const chunk of iterator) {
    ...
    data = newData;
    let patternIndex; // TODO: Remove if unused
...
constructor(chunk: Bytes): string[] {
    ...
    if (lines.length === 1 && !trailingNewline) {
      this.(buffer as any[]).push(lines[0]!);
      return [];
    }
...
constructor(bytes: Bytes): string {
...
export function _decodeChunks(chunks: string[]): string[] {
  ...
  for (const chunk of chunks) {
    (lines as any[]).push(...decoder.decode(chunk));
  }
  return lines;
}
